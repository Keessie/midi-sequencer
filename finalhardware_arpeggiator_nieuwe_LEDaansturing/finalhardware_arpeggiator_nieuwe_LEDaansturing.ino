...
const byte STOP = 252;
byte sysexStart = 0xF0;  // sysex start byte
byte sysexStop = 0xF7;   // sysex stop byte
      ...
      if ((kpd.key[i].kchar >= '0') && (kpd.key[i].kchar <= 'U')) {
        ...
        switch (kpd.key[i].kstate) {  // Report active key state : IDLE, PRESSED, HOLD, or RELEASED
          ...
          case PRESSED:
            ...
            if ((Mode ==  TIMEUNIT) && (currentKey > 0) && (currentKey < 5)) {
              timeUnit = currentKey;
            }
...
void buttonRead(int m) {
  ...
  if (m == 0) {/// if button 0 is pressed,pause sequencer
    //byte data[] = { 0xF0}; //
    //usbMIDI.sendSysEx(8, data);         // send start message
    if (sequencerPaused == true) {    // play pause gedoe
      ...
      //byte data[] = {0xF7};
      //usbMIDI.sendSysEx(8, data);// send stop message
      sequencerPaused = false;
      Sprintln("pause");
      Sprintln("stepCounter ");
      Sprintln(stepCounter);
...
void RealTimeSystem(byte realtimebyte) {
  if ((Mode <= ARP ) && (realtimebyte == 248)) {
    counter ++;
    if ( counter == (12 * timeUnit)) { // 8th notes stepswitch, counter resets
      counter = 0;
